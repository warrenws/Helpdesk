...
<%
'Created by Matthew Hull on 1/13/12
'Last Updated 6/16/14

'This is the dates stats page. 

Option Explicit

On Error Resume Next

Dim objNetwork, strUserAgent, strSQL, objNameCheckSet, strRole, objSubmittedPerDay
Dim objCompletedPerDay, objWorkedOnPerDay, objCounters, strUser, bolShowLogout
...
Sub AccessGranted
   ...
   strSQL = "SELECT SubmitDate, Count(ID) AS CountOfID" & vbCRLF
   strSQL = strSQL & "FROM Main" & vbCRLF
   strSQL = strSQL & "GROUP BY SubmitDate" & vbCRLF
   strSQL = strSQL & "ORDER BY SubmitDate DESC"
   Set objSubmittedPerDay = Application("Connection").Execute(strSQL)
   
   strSQL = "SELECT LastUpdatedDate, Count(ID) AS CountOfID" & vbCRLF
   strSQL = strSQL & "FROM Main" & vbCRLF
   strSQL = strSQL & "GROUP BY LastUpdatedDate, Status" & vbCRLF
   strSQL = strSQL & "HAVING Status='Complete'" & vbCRLF
   strSQL = strSQL & "ORDER BY LastUpdatedDate DESC"
   Set objCompletedPerDay = Application("Connection").Execute(strSQL)

   strSQL = "SELECT UpdateDate, Count(Ticket) AS CountOfTicket" & vbCRLF
   strSQL = strSQL & "FROM (SELECT UpdateDate, Ticket" & vbCRLF
   strSQL = strSQL & "FROM Log" & vbCRLF
   strSQL = strSQL & "GROUP BY UpdateDate, Ticket)" & vbCRLF
   strSQL = strSQL & "GROUP By UpdateDate" & vbCRLF
   strSQL = strSQL & "ORDER BY UpdateDate DESC"
   set objWorkedOnPerDay = Application("Connection").Execute(strSQL)
...
<% End If %>
   ...
   By Date<font class="separator"> | </font> 
...
<% End If %>    
   ...
   <table width="750">
      <tr><td valign="top" align="left">
         <table>
            <tr><th colspan="2" align="center">Submitted Each Day</th></tr>
            <tr><th>Day</th><th>Tickets</th></tr>
            
         <% Do Until objSubmittedPerDay.EOF %>
            <tr>
               <td class="showborders"><a href="view.asp?date=<%=objSubmittedPerDay(0)%>&type=submitted"><%=objSubmittedPerDay(0)%></a></td>
               <td class="showborders" align="center"><%=objSubmittedPerDay(1)%></td>
            </tr>
         <%    objSubmittedPerDay.MoveNext
            Loop %>
         </table>
      </td><td valign="top" align="left">
         <table>
            <tr><th colspan="2" align="center">Worked On Each Day</th></tr>
            <tr><th>Day</th><th>Tickets</th></tr>
            
         <% Do Until objWorkedOnPerDay.EOF %>
            <tr>
               <td class="showborders"><a href="view.asp?date=<%=objWorkedOnPerDay(0)%>&type=workedon"><%=objWorkedOnPerDay(0)%></a></td>
               <td class="showborders" align="center"><%=objWorkedOnPerDay(1)%></td>
            </tr>
         <%    objWorkedOnPerDay.MoveNext
            Loop %>
         </table>      
      </td><td valign="top" align="left">
         <table>
            <tr><th colspan="2" align="center">Closed Each Day</th></tr>
            <tr><th>Day</th><th>Tickets</th></tr>
            
         <% Do Until objCompletedPerDay.EOF %>
            <tr>
               <td class="showborders"><a href="view.asp?date=<%=objCompletedPerDay(0)%>&type=completed"><%=objCompletedPerDay(0)%></a></td>
               <td class="showborders" align="center"><%=objCompletedPerDay(1)%></td>
            </tr>
         <%    objCompletedPerDay.MoveNext
            Loop %>
         </table>    
      </td></tr>
   </table>
