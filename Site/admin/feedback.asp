...
<%
'Created by Matthew Hull on 1/19/12
...
Dim objNetwork, strUserAgent, strSQL, objNameCheckSet, strRole, objTechRating, strRating
Dim objLocationRating, strURLRoot, objOverallRating, objComments, strTech, strLocation
Dim strMessage, objCounters, bolViewAll, bolCommentsOnly, strUser, bolShowLogout
...
Sub AccessGranted
   ...
   bolViewAll = Request.QueryString("ViewAll")
   bolCommentsOnly = Request.QueryString("CommentsOnly")
   ...
   strSQL = "SELECT Tech, Avg(Rating) AS AvgOfRating, Count(Rating) AS CountOfRating" & vbCRLF
   strSQL = strSQL & "FROM Feedback" & vbCRLF
   strSQL = strSQL & "GROUP BY Tech"
   Set objTechRating = Application("Connection").Execute(strSQL)
   
   strSQL = "SELECT Location, Avg(Rating) AS AvgOfRating, Count(Rating) AS CountOfRating" & vbCRLF
   strSQL = strSQL & "FROM Feedback" & vbCRLF
   strSQL = strSQL & "GROUP BY Location"
   Set objLocationRating = Application("Connection").Execute(strSQL)
   
   If strTech <> "" Then
      strSQL = "SELECT Main.DisplayName, Feedback.Ticket, Feedback.Rating, Feedback.Comment" & vbCRLF
      strSQL = strSQL & "FROM Feedback INNER JOIN Main ON Feedback.Ticket = Main.ID" & vbCRLF
      strSQL = strSQL & "WHERE (((Feedback.Tech)='" & strTech & "'))" & vbCRLF
      strSQL = strSQL & "ORDER BY Feedback.ID DESC;" & vbCRLF
      Set objComments = Application("Connection").Execute(strSQL)
      strMessage = strTech & "'s Feedback"
   End If
   
   If strLocation <> "" Then
      strSQL = "SELECT Main.DisplayName, Feedback.Ticket, Feedback.Rating, Feedback.Comment" & vbCRLF
      strSQL = strSQL & "FROM Feedback INNER JOIN Main ON Feedback.Ticket = Main.ID" & vbCRLF
      strSQL = strSQL & "WHERE (((Feedback.Location)='" & strLocation & "'))" & vbCRLF
      strSQL = strSQL & "ORDER BY Feedback.ID DESC;" & vbCRLF
      Set objComments = Application("Connection").Execute(strSQL)      
      strMessage = strLocation & "'s Feedback"
   End If
   ...
   If bolViewAll Then
      ...
      Set objComments = Application("Connection").Execute(strSQL)
      strMessage = "All Feedback (<a href=""feedback.asp?CommentsOnly=True"">Comments Only</a>)"
   End If
   
   If bolCommentsOnly Then
      strSQL = "SELECT Main.DisplayName, Feedback.Ticket, Feedback.Rating, Feedback.Comment" & vbCRLF
      strSQL = strSQL & "FROM Feedback INNER JOIN Main ON Feedback.Ticket = Main.ID" & vbCRLF
      strSQL = strSQL & "WHERE Feedback.Comment <> ''" & vbCRLF
      strSQL = strSQL & "ORDER BY Feedback.ID DESC"
      Set objComments = Application("Connection").Execute(strSQL)
      strMessage = "Comments Only (<a href=""feedback.asp?ViewAll=True"">View All</a>)"
   End If
...
<% End If %>    
   ...
   <table width="750">
      ...
      <table>
         <tr>
            <th><a href="feedback.asp">Overall Rating</a></th>
         </tr>
         <td align="center">          
         <% If objOverallRating(0) >= 0 And objOverallRating(0) < .25 Then 
               strRating = "src=""" & strURLRoot & "0star.png"""
            ElseIf objOverallRating(0) >= .25 And objOverallRating(0) < 1 Then 
               strRating = "src=""" & strURLRoot & "0star-half.png"""
            ElseIf objOverallRating(0) >= 1 And objOverallRating(0) < 1.25 Then
               strRating = "src=""" & strURLRoot & "1star.png"""
            ElseIf objOverallRating(0) >= 1.25 And objOverallRating(0) < 2 Then 
               strRating = "src=""" & strURLRoot & "1star-half.png"""
            ElseIf objOverallRating(0) >= 2 And objOverallRating(0) < 2.25 Then
               strRating = "src=""" & strURLRoot & "2star.png"""   
            ElseIf objOverallRating(0) >= 2.25 And objOverallRating(0) < 3 Then 
               strRating = "src=""" & strURLRoot & "2star-half.png"""
            ElseIf objOverallRating(0) >= 3 And objOverallRating(0) < 3.25 Then
               strRating = "src=""" & strURLRoot & "3star.png"""   
            ElseIf objOverallRating(0) >= 3.25 And objOverallRating(0) < 4 Then 
               strRating = "src=""" & strURLRoot & "3star-half.png"""
            ElseIf objOverallRating(0) >= 4 And objOverallRating(0) < 4.25 Then
               strRating = "src=""" & strURLRoot & "4star.png"""   
            ElseIf objOverallRating(0) >= 4.25 And objOverallRating(0) < 5 Then 
               strRating = "src=""" & strURLRoot & "4star-half.png"""
            ElseIf objOverallRating(0) >= 5  Then
               strRating = "src=""" & strURLRoot & "5star.png"""      
            End If %>
            <img <%=strRating%> /> (<%=Round(objOverallRating(0),2)%>)
         </td></tr>
         <tr><td>
            <%=objOverallRating(1)%>/<%=objCounters(0)%> Reviews Recieved - <%=Round((objOverallRating(1)/objCounters(0))*100,2)%>%
         </td>
      </table>
      </td></tr>
   ...
   <% If Not objTechRating.EOF Then 
         Do Until objTechRating.EOF %>
            <tr>
               <td class="showborders"><a href="feedback.asp?Tech=<%=objTechRating(0)%>"><%=objTechRating(0)%></a></td>
               <td class="showborders">
                  
               <% If objTechRating(1) >= 0 And objTechRating(1) < .25 Then 
                     strRating = "src=""" & strURLRoot & "0star.png"""
                  ElseIf objTechRating(1) >= .25 And objTechRating(1) < 1 Then 
                     strRating = "src=""" & strURLRoot & "0star-half.png"""
                  ElseIf objTechRating(1) >= 1 And objTechRating(1) < 1.25 Then
                     strRating = "src=""" & strURLRoot & "1star.png"""
                  ElseIf objTechRating(1) >= 1.25 And objTechRating(1) < 2 Then 
                     strRating = "src=""" & strURLRoot & "1star-half.png"""
                  ElseIf objTechRating(1) >= 2 And objTechRating(1) < 2.25 Then
                     strRating = "src=""" & strURLRoot & "2star.png"""   
                  ElseIf objTechRating(1) >= 2.25 And objTechRating(1) < 3 Then 
                     strRating = "src=""" & strURLRoot & "2star-half.png"""
                  ElseIf objTechRating(1) >= 3 And objTechRating(1) < 3.25 Then
                     strRating = "src=""" & strURLRoot & "3star.png"""   
                  ElseIf objTechRating(1) >= 3.25 And objTechRating(1) < 4 Then 
                     strRating = "src=""" & strURLRoot & "3star-half.png"""
                  ElseIf objTechRating(1) >= 4 And objTechRating(1) < 4.25 Then
                     strRating = "src=""" & strURLRoot & "4star.png"""   
                  ElseIf objTechRating(1) >= 4.25 And objTechRating(1) < 5 Then 
                     strRating = "src=""" & strURLRoot & "4star-half.png"""
                  ElseIf objTechRating(1) >= 5  Then
                     strRating = "src=""" & strURLRoot & "5star.png"""      
                  End If %>
                  <img <%=strRating%> /> (<%=Round(objTechRating(1),2)%>)
               </td>
               <td class="showborders" align="center"><%=objTechRating(2)%></td>
            </tr>
   <%       objTechRating.MoveNext
         Loop 
      End If %>
      </table>
      </td>
   ...
   <% If Not objLocationRating.EOF Then 
         Do Until objLocationRating.EOF %>
            <tr>
               <td class="showborders"><a href="feedback.asp?Location=<%=objLocationRating(0)%>"><%=objLocationRating(0)%></a></td>
               <td class="showborders">
                  
               <% If objLocationRating(1) >= 0 And objLocationRating(1) < .25 Then 
                     strRating = "src=""" & strURLRoot & "0star.png"""
                  ElseIf objLocationRating(1) >= .25 And objLocationRating(1) < 1 Then 
                     strRating = "src=""" & strURLRoot & "0star-half.png"""
                  ElseIf objLocationRating(1) >= 1 And objLocationRating(1) < 1.25 Then
                     strRating = "src=""" & strURLRoot & "1star.png"""
                  ElseIf objLocationRating(1) >= 1.25 And objLocationRating(1) < 2 Then 
                     strRating = "src=""" & strURLRoot & "1star-half.png"""
                  ElseIf objLocationRating(1) >= 2 And objLocationRating(1) < 2.25 Then
                     strRating = "src=""" & strURLRoot & "2star.png"""   
                  ElseIf objLocationRating(1) >= 2.25 And objLocationRating(1) < 3 Then 
                     strRating = "src=""" & strURLRoot & "2star-half.png"""
                  ElseIf objLocationRating(1) >= 3 And objLocationRating(1) < 3.25 Then
                     strRating = "src=""" & strURLRoot & "3star.png"""   
                  ElseIf objLocationRating(1) >= 3.25 And objLocationRating(1) < 4 Then 
                     strRating = "src=""" & strURLRoot & "3star-half.png"""
                  ElseIf objLocationRating(1) >= 4 And objLocationRating(1) < 4.25 Then
                     strRating = "src=""" & strURLRoot & "4star.png"""   
                  ElseIf objLocationRating(1) >= 4.25 And objLocationRating(1) < 5 Then 
                     strRating = "src=""" & strURLRoot & "4star-half.png"""
                  ElseIf objLocationRating(1) >= 5  Then
                     strRating = "src=""" & strURLRoot & "5star.png"""      
                  End If %>
                  <img <%=strRating%> /> (<%=Round(objLocationRating(1),2)%>)
               </td>
               <td class="showborders" align="center"><%=objLocationRating(2)%></td>
            </tr>
   <%       objLocationRating.MoveNext
         Loop 
      End If %>     
      </table>
      <tr><td colspan=2><hr /></td></tr>
   <% If strMessage <> "" Then %>   
      <tr><th colspan=2><%=strMessage%></th></tr>
      <tr><td valign="top" align="center">
      <table>
         <tr>
            <th>&nbsp;&nbsp;&nbsp;User&nbsp;&nbsp;&nbsp;</th>
      ...
      <% Do Until objComments.EOF 
            If objComments(2) >= 0 And objComments(2) < .25 Then 
               strRating = "src=""" & strURLRoot & "0star.png"""
            ElseIf objComments(2) >= .25 And objComments(2) < 1 Then 
               strRating = "src=""" & strURLRoot & "0star-half.png"""
            ElseIf objComments(2) >= 1 And objComments(2) < 1.25 Then
               strRating = "src=""" & strURLRoot & "1star.png"""
            ElseIf objComments(2) >= 1.25 And objComments(2) < 2 Then 
               strRating = "src=""" & strURLRoot & "1star-half.png"""
            ElseIf objComments(2) >= 2 And objComments(2) < 2.25 Then
               strRating = "src=""" & strURLRoot & "2star.png"""   
            ElseIf objComments(2) >= 2.25 And objComments(2) < 3 Then 
               strRating = "src=""" & strURLRoot & "2star-half.png"""
            ElseIf objComments(2) >= 3 And objComments(2) < 3.25 Then
               strRating = "src=""" & strURLRoot & "3star.png"""   
            ElseIf objComments(2) >= 3.25 And objComments(2) < 4 Then 
               strRating = "src=""" & strURLRoot & "3star-half.png"""
            ElseIf objComments(2) >= 4 And objComments(2) < 4.25 Then
               strRating = "src=""" & strURLRoot & "4star.png"""   
            ElseIf objComments(2) >= 4.25 And objComments(2) < 5 Then 
               strRating = "src=""" & strURLRoot & "4star-half.png"""
            ElseIf objComments(2) >= 5  Then
               strRating = "src=""" & strURLRoot & "5star.png"""      
            End If %>      
      
            <tr>
               <td class="showborders"><%=objComments(0)%></td>
               <td class="showborders" align="center"><a href="modify.asp?ID=<%=objComments(1)%>"><%=objComments(1)%></a></td>
               <td class="showborders"><img <%=strRating%> /></td>
               <td class="showborders"><%=objComments(3)%></td>
            </tr>
...
<% Else %>
      <tr><th colspan=2><a href="feedback.asp?ViewAll=True">View All Feedback</a></th></tr>
<% End If %>
   </table>
   </center>
   </body>
